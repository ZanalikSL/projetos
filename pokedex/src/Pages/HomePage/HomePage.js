...
export default function HomePage() {
  ...
  const listPokemons = pokeList.pokemons.map((pokemon) => {
    ...
    return existPokemon ? (<div></div>) : (
      <PokemonCard url={pokemon.url} pokemon={pokemon} />
    )
  })
  ...
  return (
    <ContainerHome className="home-container">
      <HomeHeader>
        <ContainerButton>
          <GoPokedexButton onClick={() => goToPokedexPage(history)}>
            Ir para Pokedex
          </GoPokedexButton>
          <GoNextPageButton onClick={() => pokeList.setValueInitial(pokeList.valueInitial + 1)}>
            Próxima Página
          </GoNextPageButton>
          {pokeList.valueInitial > 0 ? (<GoPreviousPageButton onClick={() =>
                pokeList.setValueInitial(pokeList.valueInitial - 1)}>
              Página Anterior
            </GoPreviousPageButton>) : (<div></div>)}
        </ContainerButton>
