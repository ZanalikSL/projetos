...
const PokemonDetailPage = () => {
  ...
  useEffect(() => {
    axios
      .get(`https://pokeapi.co/api/v2/pokemon/${param.pokeName}`)
      .then((response) => {
  ...
  const removePokemon = (namePokemon) => {
    const listPokedexFiltered =
      pokedexList.pokedex &&
      pokedexList.pokedex.filter((poke) => {
  ...
  return (
    <DetailPageContainer>
      <DetailPageHeader>
        <GoBackButton onClick={() => goToPreviousPage(history)}>
          Voltar
        </GoBackButton>
        <PokeNameTitle>{pokemonName}</PokeNameTitle>
        <AddRemoveButton onClick={() => verifyPokemon(pokemonName)}>
      ...
      <DetailPageMain>
        ...
        <DetailsList>
          <DetailStat>
            <h2>STATUS</h2> {detailStatList}{' '}
          </DetailStat>
          <DetailType>
            <h2>TIPO</h2> {detailTypeList}{' '}
          </DetailType>
          <DetailMove>
            <h2>ATAQUES</h2> {detailMoveList}
          </DetailMove>
        </DetailsList>
      </DetailPageMain>
    </DetailPageContainer>
